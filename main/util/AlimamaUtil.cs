...
public class AlimamaUtil
{
    ...
    public static ArrayList query_exist_apply(string user_num_id, string cookies, out string out_log)
    {
        try
        {
            string newValue = StringUtil.subString(cookies, 0, "_tb_token_=", ";");
            WebClient client = new WebClient();
            client.Headers.Clear();
            client.Headers.Add("Accept", "application/json, text/javascript, */*; q=0.01");
            client.Headers.Add("X-Requested-With", "XMLHttpRequest");
            ...
            string address = str2.Replace("{oriMemberid}", user_num_id).Replace("{timestamp}", current_time() + "").Replace("{tbtoken}", newValue);
            string content = "";
            byte[] buffer = client.DownloadData(address);
            string str5 = client.ResponseHeaders["Content-Encoding"];
            if ("gzip".Equals(str5))
            ...
            ArrayList list = query_campaign(content, user_num_id, cookies);
            out_log = "";
            return list;
        }
        catch (Exception exception)
    ...
    public static bool apply_campaign(CmsForm cmsForm, string campid, string keeperid, string applyreason_pre, string applyreason, string taoke_cookie, out string out_log, string queryToken)
    {
        try
        {
            string submit_url = "http://pub.alimama.com/pubauc/applyForCommonCampaign.json";
            if (!string.IsNullOrEmpty(queryToken))
            {
                submit_url = submit_url + "?" + queryToken;
            }
            ...
            //LogUtil.log_cms_call(cmsForm, str4);
            if (str4.Contains("\"url\":\"http"))
            {
                string code_url = StringUtil.subString(str4, 0, "\"url\":\"", "\"");
                if (!String.IsNullOrEmpty(code_url))
                {
                    bool check_code = AlimamaCodeUtil.check_code(cmsForm, 0, code_url, taoke_cookie, out out_log, out queryToken);
                    if (check_code)
                    {
                        return AlimamaUtil.apply_campaign(cmsForm, campid, keeperid, applyreason_pre, applyreason, taoke_cookie, out out_log, queryToken);
                    }
                }
            }
            //string str6 = StringUtil.subString(str4, 0, "\"message\":\"", "\"");
    ...
    public static void exitCampaign(String user_num_id,string campain_id, string cookie, out string out_log)
    {
        try
        {
            string pubCampaignid = "";
            ArrayList list_apply = AlimamaUtil.query_exist_apply(user_num_id, cookie, out out_log);
            foreach (CampaignBean class4 in list_apply)
            {
                if (class4.string_7.Equals("true"))
                {
                    pubCampaignid = class4.id;
                }
            }

            //cookie = cookie+ CookieUtil.get_cookie("http://pub.alimama.com/myunion.htm");
            ...
            ////client.Headers.Add("Accept-Language", "zh-CN,zh;q=0.8");
            //client.Headers.Add("Content-Type", "application/x-www-form-urlencoded; charset=UTF-8");
            ////client.Headers.Add("Host", "pub.alimama.com");
            ////client.Headers.Add("Origin", "http://pub.alimama.com");
            //client.Headers.Add("X-Requested-With", "XMLHttpRequest");
            ...
            //client.Headers.Add("Referer", "http://pub.alimama.com/promo/search/index.htm?queryType=2&q=https%3A%2F%2Fitem.taobao.com%2Fitem.htm%3Fid%3D547902041822");
            //client.Headers.Add("Cookie", cookie);
            string address = "http://pub.alimama.com/campaign/exitCampaign.json";
            string s = "pubCampaignid={cmpid}&_tb_token_={token}"
                .Replace("{cmpid}", pubCampaignid).Replace("{token}", newValue);
            byte[] buffer = client.UploadData(address, "POST", Encoding.UTF8.GetBytes(s));
            string content = "";
            string str5 = client.ResponseHeaders["Content-Encoding"];
            if ("gzip".Equals(str5))
            ...
            {
                out_log = "退出低佣金计划【成功】！低佣金计划ID：【" + campain_id + "】";
            }
            else
            {
                //string str6 = StringUtil.subString(content, 0, "\"message\":\"", "\"");
                out_log = "退出低佣金计划【失败】！低佣金计划ID：【" + campain_id + "】，失败原因：【" + content + "】";
            }
        }
        catch (Exception exception)
        {
    ...
    public static AlimamaClick query_taoke_click(string taoke_cookie, string num_iid, string adzoneid, string siteid, int url_type, out string out_log)
    {
        try
        {
            ...
            if (str4.Contains("\"info\":{\"message\":null,\"ok\":true},\"ok\":true"))
            {
                ...
                class2.coupon_short_url = StringUtil.subString(str4, 0, "\"couponShortLinkUrl\":\"", "\"");
    ...
    public static ArrayList query_taoke_goods_list(string url, string taoke_cookie,out string out_log)
    {
        ...
        for (int i = 0; (i = str6.IndexOf("\"tkSpecialCampaignIdRateMap\"", i)) != -1; i++)
        {
            ...
            try
            {
                class2.tkMktStatus = int.Parse(StringUtil.smethod_1(str6, i, "\"tkMktStatus\":", ",", "}"));
            }catch (Exception){}
            class2.couponEffectiveStartTime = StringUtil.subString(str6, i, "\"couponEffectiveStartTime\":\"", "\"");
            class2.couponEffectiveEndTime = StringUtil.subString(str6, i, "\"couponEffectiveEndTime\":\"", "\""); 
