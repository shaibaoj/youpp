...
namespace WeChat.NET.HTTP
{
    ...
    class WXService
    {
        ...
        //同步检查url
        private static string _synccheck_url = "https://webpush.wx.qq.com/cgi-bin/mmwebwx-bin/synccheck?sid={0}&uin={1}&synckey={2}&r={3}&skey={4}&deviceid={5}";
        //同步url
        ...
        //获取群列表
        private static string _getcontact_qun_url = "https://wx.qq.com/cgi-bin/mmwebwx-bin/webwxbatchgetcontact?type=ex&r=1454120072706";

        //上传文件url
        private static string _post_url = "https://file.wx.qq.com/cgi-bin/mmwebwx-bin/webwxuploadmedia?f=json";
        //发送img消息url
        private static string _sendmsg_img_url = "https://wx.qq.com/cgi-bin/mmwebwx-bin/webwxsendmsgimg?fun=async&f=json";
        ...
        public string SendMsg_img(string msg, string from, string to, int type, string MediaId)
        {
            string msg_json = "{{" +
            "\"BaseRequest\":{{" +
                "\"DeviceID\" : \"e441551176\"," +
            ...
            if (sid != null && uin != null)
            {
                msg_json = string.Format(msg_json, sid.Value, uin.Value, msg, from, to, type, LoginService.SKey, DateTime.Now.Millisecond, DateTime.Now.Millisecond, DateTime.Now.Millisecond, MediaId);

                byte[] bytes = BaseService.SendPostRequest(_sendmsg_img_url 
                     + "&lang=zh_CN&pass_ticket=" + LoginService.Pass_Ticket, msg_json);
